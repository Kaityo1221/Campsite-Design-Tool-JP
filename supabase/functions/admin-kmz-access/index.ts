...
function sanitizeText(value: unknown, maxLength = 500): string { return typeof value === "string" ? value.trim().slice(0, maxLength) : ""; }
function bytesToHex(bytes: Uint8Array): string { return Array.from(bytes).map((b) => b.toString(16).padStart(2, "0")).join(""); }
async function sha256(value: string): Promise<string> { const digest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(value)); return bytesToHex(new Uint8Array(digest)); }
async function requireAdminSession(supabase: any, token: string) {
  if (!token) return null;
  const tokenHash = await sha256(token); const nowIso = new Date().toISOString();
  const { data, error } = await supabase.from("admin_sessions").select("id, expires_at").eq("token_hash", tokenHash).is("revoked_at", null).gt("expires_at", nowIso).maybeSingle();
  if (error || !data) return null;
  await supabase.from("admin_sessions").update({ last_used_at: nowIso }).eq("id", data.id); return data;
}
...
function buildDeviceLabels(rows: any[], currentDeviceId: string) {
  const ids = [...new Set(rows.map((r) => r.anonymous_device_id).filter(Boolean))] as string[]; const labels = new Map<string, string>(); let n = 1;
  ids.forEach((id) => { if (currentDeviceId && id === currentDeviceId) labels.set(id, "この端末"); else labels.set(id, `端末 ${String(n++).padStart(2, "0")}`); }); return labels;
}
function creatorDisplayName(row: any): string {
  const globalName = sanitizeText(row.created_by_discord_global_name, 100); const userName = sanitizeText(row.created_by_discord_name, 100);
  if (globalName && userName && globalName !== userName) return `${globalName} (@${userName})`; if (userName) return `@${userName}`; if (globalName) return globalName; return "作成者情報なし";
}
function toPublic(row: any, labels: Map<string, string>, currentDeviceId: string) {
  return {
    id: row.id,
    siteId: row.site_id || null,
    siteAssignmentMethod: row.site_assignment_method || null,
    siteMatchScore: row.site_match_score === null || row.site_match_score === undefined ? null : Number(row.site_match_score),
...
function buildUnique(rows: any[], labels: Map<string, string>, currentDeviceId: string) {
  ...
  rows.forEach((row) => { const key = row.file_hash || row.id; if (!groups.has(key)) groups.set(key, []); groups.get(key)!.push(row); });
  return [...groups.values()].map((group) => {
    const sorted = [...group].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()); const rep = sorted[0];
    const currentCount = currentDeviceId ? sorted.filter((r) => r.anonymous_device_id === currentDeviceId).length : 0;
    return { ...toPublic(rep, labels, currentDeviceId), historyCount: sorted.length, duplicateCount: sorted.filter((r) => r.upload_status === "duplicate").length, firstActivityAt: sorted[sorted.length - 1]?.created_at || rep.created_at, lastActivityAt: rep.created_at, actionTypes: [...new Set(sorted.map((r) => r.action_type))], currentDeviceHistoryCount: currentCount, otherDeviceHistoryCount: sorted.length - currentCount, hasCurrentDeviceActivity: currentCount > 0, hasOtherDeviceActivity: sorted.length - currentCount > 0 };
  }).sort((a, b) => new Date(b.lastActivityAt).getTime() - new Date(a.lastActivityAt).getTime());
}
function summarize(rows: any[]) {
  const today = tokyoDateKey(new Date()); const sevenDaysAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
  return { history: rows.length, uniqueFiles: new Set(rows.map((r) => r.file_hash || r.id)).size, duplicateHistory: rows.filter((r) => r.upload_status === "duplicate").length, distinctDevices: new Set(rows.map((r) => r.anonymous_device_id).filter(Boolean)).size, kmzGenerateCount: rows.filter((r) => r.action_type === "kmz_generate").length, distanceCheckCount: rows.filter((r) => r.action_type === "distance_check").length, todayCount: rows.filter((r) => tokyoDateKey(r.created_at) === today).length, last7DaysCount: rows.filter((r) => new Date(r.created_at).getTime() >= sevenDaysAgo).length };
}
...
Deno.serve(async (request: Request): Promise<Response> => {
  ...
  try {
    const body = await request.json().catch(() => ({})); const sessionToken = sanitizeText(body?.sessionToken, 500); const session = await requireAdminSession(supabase, sessionToken);
    ...
    if (action === "list") {
      ...
      const { data, error } = await supabase.from("campsite_kmz_uploads")
    ...
    if (action === "download") {
      const recordId = sanitizeText(body?.recordId, 80); if (!recordId) return jsonResponse({ success: false, error: "KMZレコードIDがありません。" }, 400);
      const { data: record, error } = await supabase.from("campsite_kmz_uploads").select("id, duplicate_of, storage_bucket, storage_path, original_file_name, display_file_name, deleted_at").eq("id", recordId).maybeSingle();
      ...
      if ((!source.storage_bucket || !source.storage_path) && source.duplicate_of) {
        const { data: original } = await supabase.from("campsite_kmz_uploads").select("id, storage_bucket, storage_path, original_file_name, display_file_name, deleted_at").eq("id", source.duplicate_of).maybeSingle(); if (original && !original.deleted_at) source = original;
      }
      if (!source.storage_bucket || !source.storage_path) return jsonResponse({ success: false, error: "KMZ本体の保存先が見つかりません。" }, 404);
      const { data: signed, error: signedError } = await supabase.storage.from(source.storage_bucket).createSignedUrl(source.storage_path, SIGNED_URL_SECONDS);
      if (signedError || !signed?.signedUrl) return jsonResponse({ success: false, error: "KMZ取得URLを発行できませんでした。" }, 500);
      return jsonResponse({ success: true, signedUrl: signed.signedUrl, expiresIn: SIGNED_URL_SECONDS, fileName: source.display_file_name || source.original_file_name || "campsite.kmz", recordId: source.id });
    }
    return jsonResponse({ success: false, error: "不明な処理です。" }, 400);
  } catch (error) {
    console.error("admin-kmz-access", error); return jsonResponse({ success: false, error: "管理者KMZアクセスでエラーが発生しました。" }, 500);
  }
});
