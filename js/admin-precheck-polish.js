...
(function () {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .apd-backdrop{position:fixed;inset:0;z-index:99990;background:rgba(2,6,23,.52);display:flex;align-items:flex-end;justify-content:center;padding:0}
      .apd-sheet{width:min(760px,100%);max-height:min(62vh,620px);overflow:hidden;border:1px solid rgba(148,163,184,.22);border-bottom:0;border-radius:22px 22px 0 0;background:#07111f;color:#e2e8f0;box-shadow:0 -18px 60px rgba(0,0,0,.45);display:flex;flex-direction:column}
  ...
  function replaceText(el, from, to) {
    ...
    if (el.children.length === 0) el.textContent = text.split(from).join(to);
  }

  function relabelUi() {
    const queue = document.getElementById("adminSiteReviewList");
    if (queue) {
      const eyebrow = queue.querySelector(".asr-eyebrow");
      const heading = queue.querySelector(".asr-head h3");
      const intro = queue.querySelector(".asr-head p:not(.asr-eyebrow)");
      if (eyebrow) eyebrow.textContent = "ADMIN · PRE-CHECK";
      if (heading) heading.textContent = "🔍 提出前チェック";
      if (intro) intro.textContent = "同じサイトの提出履歴をまとめて、提出前の確認ポイントをチェックします。正式な審査は運営側で行います。";
      queue.querySelectorAll(".asr-open").forEach(button => {
        button.textContent = "チェックを開く";
        button.setAttribute("aria-label", "このサイトの提出前チェックを開く");
      });
      queue.querySelectorAll(".asr-pill,.asr-state").forEach(el => {
        if (el.children.length === 0) {
          el.textContent = (el.textContent || "")
            .replace(/審査サイト/g, "サイト")
            .replace(/審査対象/g, "チェック対象");
        }
      });
    }
    ...
    if (workspace) {
      const eyebrow = workspace.querySelector(".arw-eyebrow");
      if (eyebrow) eyebrow.textContent = "ADMIN · PRE-CHECK";
      const title = workspace.querySelector("#arwSiteTitle");
      if (title && /審査ワークスペース/.test(title.textContent || "")) title.textContent = "提出前チェック";
      workspace.querySelectorAll("[data-arw-legacy],[data-arw-inline-legacy]").forEach(button => { button.style.display = "none"; });
      workspace.querySelectorAll(".arr-panel").forEach(panel => { panel.style.display = "none"; });
    }

    const scope = document.getElementById("adminReviewScopeControl");
    if (scope) {
      const heading = scope.querySelector(".ars-head strong");
      if (heading) heading.textContent = "チェック範囲";
      const counts = scope.querySelector("[data-ars-counts]");
      if (counts) counts.innerHTML = counts.innerHTML.replace(/審査対象/g, "チェック対象");
      const help = scope.querySelector("[data-ars-help]");
      replaceText(help, "審査範囲", "チェック範囲");
    }

    document.querySelectorAll(".aar-note,.aar-foot,.apc-note").forEach(el => {
      if (el.children.length === 0) {
        el.textContent = (el.textContent || "")
          .replace(/審査範囲/g, "チェック範囲")
          .replace(/審査対象/g, "チェック対象")
          .replace(/最終審査結果/g, "正式判断");
      }
    });
  ...
  function detailPayload(key, result) {
    ...
    if (key === "spacing") {
      ...
      return {
        title: `距離の確認 ${count}組`,
        body: `<p class="apd-note">追加予定POIを含む、基準距離未満の組み合わせです。短い順に表示しています。</p>${pairRows(result.spacingPairs)}`
      };
    }
    if (key === "duplicates") {
      ...
      return {
        title: `重複候補 ${count}組`,
        body: `<p class="apd-note">1m未満の組み合わせを重複候補として表示しています。</p>${pairRows(result.duplicatePairs)}`
      };
    }
    if (key === "limits") {
    ...
    if (key === "unknown") {
      ...
      return {
        title: `未分類 ${count}件`,
        body: `<div class="apd-unknown">種類を判別できない追加POIが ${count}件あります。種類別上限の判定では、この件数分を保留しています。</div>`
      };
    }
