(() => {
  ...
  function normalizePoi(raw) {
    ...
    return {
      ...
      gameEntity,
      gameStatus,
      bridgeSource: raw.bridgeSource || 'bridge-preview-m1',
  ...
  async function importFiles(files) {
    ...
    for (const file of files) {
      try {
        ...
        console.error('[Campsite Bridge M1] import failed', file.name, error);
  ...
  function exportMerged() {
    const payload = {
      ...
      version: VERSION,
      milestone: 'M1_MULTI_SNAPSHOT_COLLECTOR',
      exportedAt: nowIso(),
      session: {
        id: state.sessionId,
    ...
    a.href = url;
    a.download = `campsite-bridge-m1-${Date.now()}.json`;
  ...
  $('resetBtn').addEventListener('click', () => {
    if (!confirm('M1の収集データをすべて消去しますか？')) return;
    state = emptyState();
    saveState();
    render();
  });
  ...
  window.CampsiteBridgeM1 = Object.freeze({
    version: VERSION,
