(function () {
  ...
  function ensureIosKmzSaveModal() {
    ...
    card.innerHTML = `
      <div style="font-size:20px;font-weight:900;color:#f8fafc;">✅ KMZ生成完了</div>
      ...
      <button id="campsiteIosKmzSaveButton" type="button" style="width:100%;min-height:52px;margin-top:16px;border:1px solid #38bdf8;border-radius:14px;background:#0ea5e9;color:white;font-size:16px;font-weight:900;">📁 KMZとして保存</button>
      <div id="campsiteIosKmzSaveStatus" style="min-height:20px;margin-top:8px;font-size:12px;line-height:1.5;color:#cbd5e1;"></div>
      <button id="campsiteIosKmzCloseButton" type="button" style="width:100%;min-height:44px;margin-top:8px;border:1px solid rgba(148,163,184,.35);border-radius:12px;background:rgba(30,41,59,.75);color:#cbd5e1;font-weight:800;">閉じる</button>
    `;

    modal.appendChild(card);
    document.body.appendChild(modal);

    card.querySelector("#campsiteIosKmzCloseButton")?.addEventListener("click", () => {
    ...
    HTMLAnchorElement.prototype.click = function () {
      if (isKmzFileName(this.download)) {
        ...
        if (sourceBlob instanceof Blob) {
          ...
          if (isIosFamily() && typeof navigator.share === "function" && typeof File === "function") {
            const shareFile = new File([fixedBlob], this.download, { type: KMZ_MIME });
            const shouldOpenCompletionModal = /_円差分更新\.kmz$/i.test(shareFile.name);
            let canShare = true;
            ...
            if (canShare) {
              ...
              if (fileNameBox) fileNameBox.textContent = shareFile.name;
              if (status) status.textContent = "";

              if (saveButton) {
                saveButton.onclick = async () => {
                  saveButton.disabled = true;
                  if (status) status.textContent = "共有シートを開いています…";

                  try {
                    await navigator.share({ files: [shareFile] });
                    if (status) status.textContent = "共有が完了しました。";
                    setTimeout(() => {
                      modal.style.display = "none";
                      if (shouldOpenCompletionModal && typeof window.openKmzCompleteModal === "function") {
          ...
          setTimeout(() => {
            ...
            if (this.href === fixedUrl) this.href = sourceUrl;
          }, 10000);
        }
      }
  ...
  function showUploadFailure(targetElement) {
    ...
    const oldMessage = targetElement.querySelector(".server-upload-note");
    if (oldMessage) oldMessage.remove();
    const message = document.createElement("div");
    message.className = "server-upload-note";
    message.textContent = "※解析データを送信できませんでした。";
    targetElement.appendChild(message);
  }

  async function uploadCampsiteFile(options) {
    const { file, blob, fileName, actionType, parkName, metadata = {}, errorTarget = null } = options || {};
    const normalizedUploadFileName = normalizeFileName(fileName);
    const uploadFile = file || (blob
      ? new File([blob], normalizedUploadFileName, {
          type: isKmzFileName(normalizedUploadFileName) ? KMZ_MIME : (blob.type || "application/octet-stream")
        })
      : null);

    if (!(uploadFile instanceof File)) throw new Error("アップロード対象のファイルがありません。");
    if (actionType !== "kmz_generate" && actionType !== "distance_check") {
      throw new Error("アップロード種別が正しくありません。");
    }
    if (!window.campsiteSupabase || !window.campsiteSupabase.functions) {
      throw new Error("Supabaseクライアントが初期化されていません。");
    }

    const formData = new FormData();
    formData.append("file", uploadFile);
    formData.append("original_file_name", normalizeFileName(fileName || uploadFile.name));
    formData.append("anonymous_device_id", getAnonymousDeviceId());
    formData.append("action_type", actionType);
    formData.append("park_name", normalizeParkName(parkName));
    ...
    try {
      const { data, error } = await window.campsiteSupabase.functions.invoke(FUNCTION_NAME, { body: formData });
      if (error) throw error;
      if (!data || data.success !== true) throw new Error(data?.error || "サーバーへの送信に失敗しました。");
      return data;
    } catch (error) {
...
import("./poi-spacing-config.js?v=3")
  .then(() => Promise.all([
    ...
    import("./poi-spacing-policy-filter.js?v=6"),
    import("./poi-spacing-policy-ui.js?v=6")
  ]))
  .then(() => import("./poi-spacing-kmz50-guard.js?v=4"))
  .then(() => import("./poi-spacing-kmz-preserve.js?v=4"))
  .then(() => import("./poi-spacing-kmz-diff.js?v=3"))
  .catch(error => {
