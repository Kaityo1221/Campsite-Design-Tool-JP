(()=>{
  function init(){
    ...
    const SYN={消す:['削除','ゴミ箱'],削除:['消す','ゴミ箱'],出ない:['表示されない','見えない'],表示されない:['出ない','見えない'],場所:['位置','座標'],位置:['場所','座標'],座標:['緯度','経度','位置'],マイマップ:['my maps','google my maps','mymaps'],スポンサー:['スポンサード','sponsored'],スポンサード:['スポンサー','sponsored'],ポケストップ:['poi','スポット'],ジム:['poi','スポット'],kmz:['kml','ファイル'],csv:['ファイル']};
    ...
    function thinking(cb){add('確認しています…',false);const row=m.lastElementChild;setTimeout(()=>{if(row)row.remove();cb()},650)}
    ...
    function score(v,t){const ws=words(t),qq=v.q.toLowerCase(),aa=v.a.toLowerCase();let n=0;const hit=[];ws.forEach(w=>{const x=w.toLowerCase();if(qq.includes(x)){n+=6;hit.push(w)}if(aa.includes(x)){n+=2;hit.push(w)}});if(step&&((step==='mymaps'&&v.c==='mymaps')||(step==='distance'&&v.c==='distance')||(step==='poi'&&(v.c==='poi'||v.c==='wayfarer'))))n+=5;if(c&&v.c===c)n+=3;if(rejected.has(v.id))n-=20;return {v,n,hit:[...new Set(hit)].slice(0,4)}}
    function doSearch(text){const queryText=appendSearchContext(text),z=f.map(v=>score(v,queryText)).sort((a,b)=>b.n-a.n).filter(x=>x.n>0);if(!z.length||z[0].n<4)return noMatch(queryText);clarify=0;persist();const strong=z[0].n>=12&&(!z[1]||z[0].n-z[1].n>=5),pick=strong?z.slice(0,1):z.slice(0,3);add(strong?'関連性の高い案内が見つかりました。':'近い内容の案内はこちらです。',false,pick.map(x=>({label:x.v.q,onClick:()=>faq(x.v)})),pick.map(x=>x.hit.join('・')).filter(Boolean).join(' / '))}
    ...
    function noMatch(queryText){clarify++;persist();if(clarify<=2){add(clarify===1?'もう少し状況を教えてください。どの画面・機能で困っていますか？':'ありがとうございます。もう一点だけ、表示されている内容や、直前に行った操作を教えてください。',false,[{label:'Wayfarer Map',onClick:()=>clarificationChoice('Wayfarer Map','wayfarer')},{label:'Google My Maps',onClick:()=>clarificationChoice('Google My Maps','mymaps')},{label:'距離チェック',onClick:()=>clarificationChoice('距離チェック','distance')},{label:'その他・自由入力',onClick:()=>clarificationChoice('その他','other')}]);i.focus()}else inquiryConfirm(searchContext||queryText)}

    function faq(v){q=v;c=v.c;mode='faq';add(v.q,true);add(v.a,false,[{label:'解決した！',kind:'good',onClick:()=>rate(true)},{label:'まだ困ってる',kind:'bad',onClick:()=>rate(false)},{label:'別の質問を見る',onClick:()=>cat(v.c)}])}
    async function rate(ok){if(!q)return;const x=q;await save({type:'question',category:x.c,faq_id:x.id,resolved:ok,content:(ok?'FAQ回答で解決: ':'FAQ回答で解決しなかった: ')+x.q});if(ok){clarify=0;searchContext='';persist();add('解決できてよかったです。ほかにお困りのことがあれば、いつでもご利用ください。',false,[{label:'最初に戻る',onClick:home}])}else{rejected.add(x.id);const baseContext=[searchContext,x.q].filter(Boolean).join(' ').trim();persist();add('承知しました。次の方法を選択してください。',false,[{label:'別の方法を試す',onClick:()=>{startTextSearch(baseContext);add('別の候補を探します。状況を短く入力してください。',false);i.focus()}},{label:'もう少し状況を伝える',onClick:()=>{startTextSearch(baseContext);add('追加の状況を入力してください。これまでの内容と合わせて確認します。',false);i.focus()}},{label:'問い合わせる',onClick:()=>inquiryConfirm(baseContext)}])}}
    ...
    async function submit(){const t=i.value.trim();if(!t)return;i.value='';add(t,true);if(imp){const ok=await save({type:'improvement',category:c||'other',content:t});add(ok?'ご要望を受け付けました。今後の改善に活用します。':'送信に失敗しました。もう一度お試しください。',false,[{label:'最初に戻る',onClick:home}]);imp=false;return}if(mode==='inquiryEdit'){inquiryConfirm(t);return}if(mode==='inquiry'){sendInquiry(t);return}thinking(()=>doSearch(t))}
