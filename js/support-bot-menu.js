(()=>{
  function init(){
    ...
    function faq(v,auto=false,hit=[]){q=v;c=v.c;mode='faq';if(auto)add('ご質問は「'+v.q+'」として案内します。',false,null,hit&&hit.length?'一致: '+hit.join('・'):null);else add(v.q,true);add(v.a,false,[{label:'解決した！',kind:'good',onClick:()=>rate(true)},{label:'まだ困ってる',kind:'bad',onClick:()=>rate(false)},{label:'別の質問を見る',onClick:()=>cat(v.c)}])}
