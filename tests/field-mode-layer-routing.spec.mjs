...
function folderPointNames(kmlText,folderName){
  ...
  if(!match)return[];
  return [...match[1].matchAll(/<Placemark>[\s\S]*?<name>([^<]+)<\/name>[\s\S]*?<Point>/g)].map(item=>item[1]);
}

test('通常保存は新規POIを種類ごとの正式レイヤーへ振り分ける',async({page})=>{
...
test('活動範囲込み保存でも正式POIレイヤーを維持する',async({page})=>{
  await openFieldMode(page);
  await addAllThreeTypes(page);
