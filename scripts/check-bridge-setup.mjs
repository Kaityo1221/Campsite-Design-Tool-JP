...
const androidRollbackXpiPath = 'downloads/campsite-bridge-android-0.3.4.xpi';
const androidRollbackXpi = fs.readFileSync(androidRollbackXpiPath);
const androidRollbackXpiSha256 = crypto.createHash('sha256').update(androidRollbackXpi).digest('hex');
const androidXpiPath = 'downloads/campsite-bridge-android-0.3.5.xpi';
...
has(html, 'bridge-setup-config.js?v=4');
has(html, 'bridge-setup.js?v=4');
...
has(config, "androidExtensionUrl: './downloads/campsite-bridge-android-0.3.5.xpi'");
...
assert.equal(androidRollbackXpi.length, 34607, 'Android rollback 0.3.4 XPI size mismatch');
assert.equal(androidRollbackXpiSha256, '13e57ace3468832c171df06bb92c983d66a22bc0f390c0ff15d0df3deff741b6', 'Android rollback 0.3.4 XPI SHA-256 mismatch');
assert.equal(androidXpi.length, 35164, 'Android signed 0.3.5 XPI size mismatch');
assert.equal(androidXpiSha256, 'f5c7d300c6cdd82758f88f651116c1b7af42863ef03e80f8ac89e39afa0fa7b7', 'Android signed 0.3.5 XPI SHA-256 mismatch');
assert.equal(androidXpi.readUInt32LE(0), 0x04034b50, 'Android signed 0.3.5 output is not an XPI/ZIP');
assert.ok(androidXpi.includes(Buffer.from('manifest.json')), 'Android signed 0.3.5 must contain manifest.json');
assert.ok(androidXpi.includes(Buffer.from('META-INF/mozilla.rsa')), 'Android signed 0.3.5 must contain Mozilla RSA signature');
assert.ok(androidXpi.includes(Buffer.from('META-INF/cose.sig')), 'Android signed 0.3.5 must contain COSE signature');
