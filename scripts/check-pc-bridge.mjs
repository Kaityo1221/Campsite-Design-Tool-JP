...
assert.ok(content.includes("type: 'CAMPSITE_BRIDGE_POI_V1'"));
assert.ok(content.includes("data.type === 'CAMPSITE_BRIDGE_READY_V1'"));
assert.ok(content.includes("data.type === 'CAMPSITE_BRIDGE_ACK_V1'"));
assert.ok(content.includes("event.origin !== RECEIVER_ORIGIN"));
assert.ok(content.includes("String(data.handshakeId || '') !== handshakeId"));
assert.ok(content.includes("window.open('about:blank'"));
assert.ok(content.includes('Wayfarer Mapを表示してからもう一度お試しください'));
assert.ok(content.includes('Wayfarerはこのまま使えます'));
