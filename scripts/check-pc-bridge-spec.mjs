...
const fencedCode = [...spec.matchAll(/```[\s\S]*?```/g)].map(match => match[0]).join('\n');
assert.equal(/CAMPSITE_BRIDGE_PC_/i.test(fencedCode), false, 'PC must not define a PC-only protocol');
